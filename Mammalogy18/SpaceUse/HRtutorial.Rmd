...
#now set the working directory
 setwd("~/Dropbox/R/Classes/Mammalogy/Mammalogy18/SpaceUse")
```
...
```{r}
library(readr)
CapeBuffalo <- read_csv("Data/CapeBuffalo/Kruger African Buffalo, GPS tracking, South Africa.csv")
```
...
Second, we can calculate the MCP estimates for each buffalo.  Often, when we estimate MCP or any other home range, we like to examine the home range size with a variety of the whole data set.
...
#and write mcp to shapefile
writeOGR(mcp_est100, wd, "MCP100", driver = "ESRI Shapefile")
writeOGR(mcp_est90, wd, "MCP90", driver = "ESRI Shapefile")
writeOGR(mcp_est80, wd, "MCP80", driver = "ESRI Shapefile")
writeOGR(mcp_est70, wd, "MCP70", driver = "ESRI Shapefile")
```
...
Now let's estimate the home range from the Utility Distrubition
...
wd<-getwd()
writeOGR(homeranges, wd, "KDEhomeranges", driver = "ESRI Shapefile")
```
...
```
Now fixt the column names to avoid confusion
```{r}
